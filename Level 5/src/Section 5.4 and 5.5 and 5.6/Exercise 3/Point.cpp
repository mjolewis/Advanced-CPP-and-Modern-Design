...
/**
 * Unary function object using a custom hash function.
 * @param point The object from which the hash value will be determined
 * @return The hash value of the specified Point
 */
std::size_t Point::operator()(const Point& point) const noexcept
{
    auto val = (unsigned long) (point.xCoordinate + point.yCoordinate);
    return val % 7919ul;
}

/**
 * Binary function object to determine equality between two points
 * @param lhs An initial point being tested for equality with the other point
 * @param rhs The other point being test for equality against the initial point
 * @return True of the two points are equal. Otherwise false
 */
bool Point::operator()(const Point& lhs, const Point& rhs) const noexcept
{
