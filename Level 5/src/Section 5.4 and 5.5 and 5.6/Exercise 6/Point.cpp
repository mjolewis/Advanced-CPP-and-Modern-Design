 ...
 * @return A std::size_t representing the hash_value of the specified point
 */
std::size_t hash_value(std::size_t seed, const Point& point)
{
    boost::hash_combine(seed, point.X());
    boost::hash_combine(seed, point.Y());
    return seed;
...
{
    return (rhs.xCoordinate == lhs.xCoordinate && rhs.yCoordinate ==lhs.yCoordinate);
}

std::size_t Point::operator()(const Point &s) const
{
    std::size_t seed = 0;
    return hash_value(seed, s);
}
