//
// Illustrate core functionality of Boost Signal and Slots 2 using slot groups
// Key observation : Slots can be placed into groups that dictate their orderings
// regardless of their connection order
//
...
void test_signals_and_slots()
{
    ...
    signal.connect(1, boost::ref(thirdSlot));
...
void test_signals_and_slots_tree()
{
    ...
    signalA.connect(1, boost::ref(signalB));
    signalB.connect(2, boost::ref(signalC));
    signalC.connect(3, boost::ref(signalD));

    // Connect Slots to Signals
    signalB.connect(4, boost::ref(slotB));
    signalC.connect(1, boost::ref(slotC));
    signalD.connect(3, boost::ref(slotD1));
    signalD.connect(2, boost::ref(slotD2));
