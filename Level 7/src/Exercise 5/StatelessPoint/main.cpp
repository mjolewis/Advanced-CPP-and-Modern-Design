...
void test_ExactDistance_StatelessPoint()
{
    ...
    DistanceStrategy distanceStrategy{exactStrategy};

    Point p1{1, 1};
    p1.setDistanceStrategy(distanceStrategy);
    Point p2{4, 5};

    double exactDistance = p1.distance(p2);

    std::cout << "Stateless Point : Exact Distance = " << exactDistance << std::endl;
}

// Supplies a generic Strategy to a Point via a universal function wrapper
// that calculates approximate distance between two points
void test_ApproximateDistance_StatelessPoint()
{
    ...
    DistanceStrategy distanceStrategy{approximateStrategy};
    ...
    std::cout << "Stateless Point : Approximate Distance = " << approximateDistance << std::endl;
}
...
void test_Concurrent_Strategies()
{
    ...
    DistanceStrategy distanceStrategy1{exactStrategy};

    Point p1{1, 1};
    p1.setDistanceStrategy(distanceStrategy1);
    Point p2{4, 5};
    ...
    DistanceStrategy distanceStrategy2{approximateStrategy};

    Point p3{1, 1};
    p3.setDistanceStrategy(distanceStrategy2);
    Point p4{4, 5};
...
int main()
{
    test_ExactDistance_StatelessPoint();
    test_ApproximateDistance_StatelessPoint();
