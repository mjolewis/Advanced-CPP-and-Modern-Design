...
/**
 * A generic Strategy that can be used to calculate the distance between two points. The DistanceStrategy
 * is an interface to a universal function wrapper that accepts two points and returns the distance
 * @param strategy A generic strategy that must be implemented with a universal function wrapper
 */
void Point::setDistanceStrategy(const DistanceStrategy &strategy)
{
    distanceStrategy = std::make_shared<DistanceStrategy>(strategy);
}
...
{
    return distanceStrategy->execute(*this, p);
}
