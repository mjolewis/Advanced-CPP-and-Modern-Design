//
// Illustrate the functionality of std::unique_lock::try_lock_for that tries
// to lock a mutex that blocks until a specific timeout duration has elapsed
// or the lock has been acquired.
//
...
void globalFunction(int iterations)
{
    ...
    while (i < iterations)
    {
        ...
        try
        {
            // Tries to lock the mutex. Blocks until specified timeout_duration has elapsed or lock is acquired
            if (console.try_lock_for(100ms))
            {
                ++i;
